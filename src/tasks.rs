...
pub fn the_next_week(rng : &mut Rng) -> RenderTask {
    ...
    }
    {
        ...
        let light : Arc<Material> =
            Arc::new(DiffuseLight(ConstantTex(7.0 * ONE3)));
        let min = center - size;
        let max = center + size;
...
#[allow(dead_code)]
pub fn cornell_smoke(rng : &mut Rng) -> RenderTask {
    let mut list : Vec<Box<Object>> = vec![];

    let white : Arc<Material> = Arc::new(Lambertian(ConstantTex(0.73 * ONE3)));

    list.append(&mut CornellBox::box_only(rng, 7.0, (152.5, 165.0)));

    list.push(box ConstantMedium {
        boundary: CornellBox::cube1(rng, &white),
        density: 0.01,
        phase_function: box Isotropic(ConstantTex(ivec3(1, 1, 1))),
    });
    list.push(box ConstantMedium {
        boundary: CornellBox::cube2(rng, &white),
        density: 0.01,
